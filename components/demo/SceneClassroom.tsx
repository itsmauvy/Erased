...
export default function SceneClassroom({ onAdvance }: Props) {
    ...
    const tick = () => {
      ...
      if (maskRef.current) {
        maskRef.current.style.background = `radial-gradient(circle 280px at ${pos.x}px ${pos.y}px,
          rgba(3,4,7,0) 0%, rgba(3,4,7,0) 20%,
          rgba(3,4,7,0.7) 55%, rgba(3,4,7,0.96) 100%)`;
      }
      if (glowRef.current) {
  ...
  const handleAdvance = () => {
    ...
    if (choice === "A") {
      updates.truthScore = 1;
      updates.trustMinseo = -1;
    } else {
      updates.trustMinseo = 1;
    }
    setStep("done");
  ...
  return (
    <div className="scene-wrap">
      ...
      {step === "explore" && (
        <button
          className="hint-btn"
          onClick={() => { setShowHint(true); setHintUsed(true); setTimeout(() => setShowHint(false), 3000); }}
        >
          HINT
        </button>
      )}
          ...
          <div className="modal-inner">
            <div className={`photo-card ${step === "photo-back" ? "flipped" : ""}`}>
              ...
              <div className="card-face card-front">
                <img src="/clues/detail-photo-front.jpg" alt="단체사진 앞면" />
                <button className="flip-btn" onClick={() => setStep("photo-back")}>
                  뒤집기 ↺
                </button>
              </div>
              {/* 뒷면 */}
              <div className="card-face card-back">
                <img src="/clues/detail-photo-back.jpg" alt="단체사진 뒷면" />
                <button className="flip-btn" onClick={() => setStep("photo-front")}>
                  앞면 보기 ↺
                </button>
              </div>
            </div>
      ...
      {(step === "chosen" || step === "done") && choice && (
        <div className="chat-panel">
          ...
          <PhoneMessage
            sender="나"
            text={choice === "A" ? "왜 얼굴이 지워져 있어?" : "이 사진 누가 찍었어?"}
            align="right"
            delay={400}
          />
          <PhoneMessage
            sender="민서"
            text={choice === "A" ? "네가 제일 먼저 지웠잖아." : "방송실에 가면 들을 수 있어."}
            delay={1200}
          />
          <div style={{ textAlign: "center", marginTop: 20 }}>
            <button className="move-btn" onClick={handleAdvance}>
              방송실로 이동
            </button>
          </div>
        </div>
      ...
      <style jsx>{`
        ...
        .bg {
          ...
          background: url('/bg/scene-classroom.jpg') center/cover no-repeat;
        }
        .mask-layer {
        ...
        .glow-layer {
          pointer-events: none; position: fixed; inset: 0; z-index: 11;
        ...
        .hotspot.hint-active {
          box-shadow: 0 0 0 6px rgba(205,178,122,0.6), 0 0 30px 10px rgba(205,178,122,0.3);
